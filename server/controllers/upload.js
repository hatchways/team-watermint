...
exports.deleteProfilePhoto = asyncHandler(async (req, res, next) => {
  ...
  if (!profile) {
    ...
    s3.deleteObject(params, (error) => {
      if (error) {
        throw res.status(500).json({ error: { message: "An unexpected error occurred. Please try again" } });
      }
    });

    if (!res.headersSent) {
      const updatedProfile = await Profile.findOneAndUpdate({ userId: userId }, { photo: '' }, { new: true });
